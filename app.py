...
if base:
    # Railway PostgreSQL fix: newer SQLAlchemy needs postgresql:// not postgres://
    ...
    db_path = os.path.abspath(os.path.join(app.instance_path, "guerrero_dental.db"))
...
class Patient(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id,
            "patient_id": self.patient_id, "patientId": self.patient_id,
            "name": self.name,
            "email": self.email, "phone": self.phone, "dob": self.dob,
            "address": self.address, "status": self.status,
            "last_visit": self.last_visit, "lastVisit": self.last_visit,
            "notes": self.notes,
...
class Appointment(db.Model):
    ...
    notes        = db.Column(db.Text)
    doctor_id    = db.Column(db.Integer, db.ForeignKey("staff_users.id"), nullable=True)
    ...
    def to_dict(self):
        return {
            "id": self.id,
            "patient_id": self.patient_id, "patientId": self.patient_id,
            "patient_name": self.patient_name, "patientName": self.patient_name,
            "type": self.type, "date": self.date, "time": self.time,
            "status": self.status, "notes": self.notes,
            "doctor_id": self.doctor_id, "doctorId": self.doctor_id,
            "doctor_name": self.doctor_name, "doctorName": self.doctor_name,
        }
...
def _doctor_complaint_filter(q, user):
    """Filter inquiries assigned to the logged-in doctor."""
    from sqlalchemy import or_
    return q.filter(or_(Complaint.assigned_to == user.id, Complaint.assigned_doctor == user.name))


def _doctor_appointment_filter(q, user):
    from sqlalchemy import or_
    return q.filter(or_(Appointment.doctor_id == user.id, Appointment.doctor_name == user.name))


def _doctor_has_conflict(doctor_id, doctor_name, date, time, exclude_appointment_id=None):
    if not date or not time or (not doctor_id and not doctor_name):
        return False
    ...
    if doctor_id:
        q = q.filter((Appointment.doctor_id == doctor_id) | (Appointment.doctor_name == doctor_name))
    else:
...
def ensure_schema():
    ...
    if "appointments" in insp.get_table_names():
        ...
        if "doctor_id" not in cols:
            db.session.execute(text("ALTER TABLE appointments ADD COLUMN doctor_id INTEGER"))
        if "doctor_name" not in cols:
            db.session.execute(text("ALTER TABLE appointments ADD COLUMN doctor_name VARCHAR(120)"))
        db.session.commit()
...
def admin_create_user():
    ...
    if creator.role == "admin" and role != "nurse":
        return jsonify({"message": "Doctors can only create nurse accounts."}), 403
    if role not in ("superadmin", "admin", "nurse"):
        return jsonify({"message": "Invalid role."}), 400
    if len(data["password"]) < 8:
        return jsonify({"message": "Password must be at least 8 characters."}), 400
...
def admin_update_user(uid):
    ...
    if editor.role == "admin" and user.role != "nurse":
        return jsonify({"message": "Doctors can only edit nurse accounts."}), 403
    data = request.get_json(silent=True)
    if not data or not isinstance(data, dict):
        return jsonify({"message": "Request body must be valid JSON."}), 400
    if "name"  in data: user.name = data["name"].strip()
...
def admin_delete_user(uid):
    ...
    editor = get_current_user()
    if editor.role == "admin" and user.role != "nurse":
        return jsonify({"message": "Doctors can only delete nurse accounts."}), 403
    if editor.id == uid:
        return jsonify({"message": "Cannot delete your own account."}), 400
    db.session.delete(user)
    db.session.commit()
...
def submit_complaint():
    ...
    if phone and email:
        exists = Complaint.query.filter(
            Complaint.phone == phone,
            Complaint.email != None,
            Complaint.email != "",
            db.func.lower(Complaint.email) == email
        ).first()
        if exists:
            return jsonify({"message": "An inquiry already exists with this phone number and email."}), 409

    c = Complaint(
        name=data["name"], phone=phone, email=data.get("email"),
...
def list_complaints():
    ...
    q = Complaint.query
    if user.role == "admin":
        q = _doctor_complaint_filter(q, user)
...
@require_role("superadmin", "admin", "nurse")
def list_doctors():
    """Active doctor accounts (role=admin) for inquiry assignment."""
    doctors = StaffUser.query.filter_by(role="admin", is_active=True).order_by(StaffUser.name).all()
    return jsonify([{"id": d.id, "name": d.name, "email": d.email} for d in doctors])


@app.route("/api/complaints/<int:cid>", methods=["PUT"])
@require_role("superadmin", "nurse")
def update_complaint(cid):
    c    = Complaint.query.get_or_404(cid)
    data = request.get_json()
    if "status" in data:
        if data["status"] == "scheduled":
            return jsonify({"message": "Set a date and time via Convert to Patient before marking as scheduled."}), 400
        c.status = data["status"]
    if "staffNotes" in data: c.staff_notes      = data["staffNotes"]
    if "assignedTo" in data:
        c.assigned_to = data["assignedTo"] or None
        if c.assigned_to:
            doc = StaffUser.query.get(c.assigned_to)
            if doc and doc.role == "admin":
                c.assigned_doctor = doc.name
    if "assignedDoctor" in data:
        c.assigned_doctor = data["assignedDoctor"] or None
        if c.assigned_doctor and not c.assigned_to:
            doc = StaffUser.query.filter_by(name=c.assigned_doctor, role="admin", is_active=True).first()
            if doc:
                c.assigned_to = doc.id
...
def list_patients():
    ...
    if user.role == "admin":
        ...
        assigned = Complaint.query.filter(
            or_(Complaint.assigned_to == user.id, Complaint.assigned_doctor == user.name)
        ).all()
        phones = {c.phone for c in assigned if c.phone}
        emails = {(c.email or "").lower() for c in assigned if c.email}
        names = {(c.name or "").lower() for c in assigned}
        appt_ids = {
            a.patient_id for a in _doctor_appointment_filter(Appointment.query, user).all()
        }
        patients = Patient.query.order_by(Patient.created_at.desc()).all()
        filtered = [
            p for p in patients
            if p.id in appt_ids
            or (p.phone and p.phone in phones)
            or (p.email and p.email.lower() in emails)
            or (p.name and p.name.lower() in names)
        ]
...
def delete_patient(pid):
    p = Patient.query.get_or_404(pid)
    # Delete associated complaints that were converted from this patient
    Complaint.query.filter(
        Complaint.phone == p.phone,
        Complaint.email == p.email,
        Complaint.status.in_(["scheduled", "in_review"])
    ).delete()
    db.session.delete(p)
    db.session.commit()
...
def list_appointments():
    ...
    q = Appointment.query
    if user.role == "admin":
        q = _doctor_appointment_filter(q, user)
    rows = q.order_by(Appointment.date.desc(), Appointment.time).all()
...
def create_appointment():
    ...
    doctor_id = data.get("doctorId") or data.get("assignedTo")
    doctor_name = data.get("doctorName") or data.get("assignedDoctor")
    user = get_current_user()
    if user.role == "admin" and not doctor_id:
        doctor_id = user.id
        doctor_name = user.name
    if doctor_id and not doctor_name:
        doc = StaffUser.query.get(doctor_id)
        if doc:
            doctor_name = doc.name
    appt_time = _normalize_time(data.get("time"))
    if _doctor_has_conflict(doctor_id, doctor_name, data.get("date"), appt_time):
        return jsonify({"message": "Selected doctor already has an appointment at that date and time."}), 409
    a = Appointment(
        patient_id=p.id, patient_name=p.name,
        type=data.get("type"), date=data.get("date"),
        time=appt_time, status="scheduled", notes=data.get("notes", ""),
...
def update_appointment(aid):
    ...
    for f in ["type","date","time","status","notes"]:
        if f in data:
            if f == "time":
                setattr(a, f, _normalize_time(data[f]))
            else:
                setattr(a, f, data[f])

    if "doctorId" in data or "assignedTo" in data:
        doctor_id = data.get("doctorId") or data.get("assignedTo")
        a.doctor_id = doctor_id or None
    if "doctorName" in data or "assignedDoctor" in data:
        doctor_name = data.get("doctorName") or data.get("assignedDoctor")
        a.doctor_name = doctor_name or None

    if a.doctor_id and not a.doctor_name:
        doc = StaffUser.query.get(a.doctor_id)
        if doc:
            a.doctor_name = doc.name
    elif a.doctor_name and not a.doctor_id:
        doc = StaffUser.query.filter_by(name=a.doctor_name, role="admin", is_active=True).first()
        if doc:
            a.doctor_id = doc.id

    new_date = a.date
    new_time = a.time
    if _doctor_has_conflict(a.doctor_id, a.doctor_name, new_date, new_time, exclude_appointment_id=a.id):
        return jsonify({"message": "Selected doctor already has an appointment at that date and time."}), 409

    db.session.commit()
    
    # Send confirmation email when appointment status changes to "confirmed"
    if old_status != "confirmed" and a.status == "confirmed":
        try:
            p = Patient.query.get(a.patient_id)
            if p and p.email:
                doctor_str = f" with {a.doctor_name}" if a.doctor_name else ""
                date_str = a.date if a.date else "TBD"
                time_str = a.time if a.time else "TBD"
                
                email_subject = f"Appointment Confirmed - Guerrero Dental"
                email_body = f"""Dear {p.name},

We are pleased to confirm your appointment at Guerrero Dental.

Appointment Details:
• Date: {date_str}
• Time: {time_str}
• Service: {a.type or 'Consultation'}
{doctor_str}
...
📞 0917-5850-158
📧 info@guerrerodental.com

Thank you for choosing Guerrero Dental!

Warm regards,
Guerrero Dental Team"""
                
                send_email(p.email, email_subject, email_body)
...
def export_patient_pdf(pid):
    ...
    elements.append(Paragraph(f"Patient Dental Chart: {patient.name}", title_style))
    ...
    doc.build(elements, onFirstPage=_draw_pdf_brand, onLaterPages=_draw_pdf_brand)
    buffer.seek(0)
    filename = f"patient_dental_chart_{patient.patient_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    return send_file(buffer, mimetype='application/pdf', as_attachment=True, download_name=filename)


@app.route('/gd-admin/patient/<int:pid>/chart')
def patient_chart_view(pid):
    # Serve chart page; authentication is handled client-side in the UI.
    p = Patient.query.get_or_404(pid)
    return render_template('patient_chart.html', patient=p.to_dict())
...
def seed():
    ...
    ad = StaffUser(name="Dr. Jasond Guerrero-Ferrer", email="admin@guerrerodental.com", role="admin", is_active=True)
...
def convert_inquiry(cid):
    ...
    # Always create appointment when converting inquiry (mandatory)
    ...
    ok, err = _validate_appointment_date_for_urgency(appt_date, c.urgency or "routine")
    if not ok:
        return jsonify({"message": err}), 400
    
    # Ensure doctor assignment
    doctor_id = c.assigned_to
    doctor_name = c.assigned_doctor
    
    if not doctor_id and not doctor_name:
        return jsonify({"message": "Inquiry must be assigned to a doctor before conversion."}), 400
    
    # If only doctor_name is set, find the doctor_id
    if doctor_name and not doctor_id:
        doc = StaffUser.query.filter_by(name=doctor_name, role="admin", is_active=True).first()
        if doc:
            doctor_id = doc.id
    
    # If only doctor_id is set, find the doctor_name
    if doctor_id and not doctor_name:
        doc = StaffUser.query.get(doctor_id)
        if doc:
            doctor_name = doc.name
    
    if _doctor_has_conflict(doctor_id, doctor_name, appt_date, appt_time):
        return jsonify({"message": "Selected doctor already has an appointment at that date and time."}), 409
    ...
    appt = Appointment(
        ...
        time=appt_time,
        status="scheduled", 
        notes=f"Converted from inquiry: {c.description or ''}",
        doctor_id=doctor_id, 
    ...
    c.status = "scheduled"
    c.staff_notes = (c.staff_notes or "") + f"\n[Converted to patient {patient.patient_id} - Appointment created]"
    ...
    return jsonify({
        ...
        "inquiry": c.to_dict(),
        "message": f"Patient record created and appointment scheduled for {doctor_name}."
    }), 200
...
def _validate_appointment_date_for_urgency(appt_date, urgency):
    """Enforce scheduling windows based on inquiry urgency."""
    if not appt_date:
        return True, None
    today = datetime.now().strftime("%Y-%m-%d")
    urgency = (urgency or "routine").lower()
    if urgency == "urgent" and appt_date != today:
        return False, "Urgent patients must be scheduled for today."
    if urgency == "soon":
        week_out = (datetime.now() + timedelta(days=7)).strftime("%Y-%m-%d")
        if appt_date < week_out:
            return False, "Soon patients must be scheduled at least one week from today."
    if appt_date < today:
